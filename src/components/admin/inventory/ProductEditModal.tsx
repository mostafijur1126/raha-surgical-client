...
export function ProductEditModal({
  ...
  const [rxRequired, setRxRequired] = useState(product.rxRequired);
  ...
  const [tiers, setTiers] = useState<EditPricingTier[]>(
    product.pricingTiers.length > 0
      ? product.pricingTiers.map((t) => ({ id: crypto.randomUUID(), ...t }))
      : [createTier("Single Piece", 1, product.baseSku)],
  );

  // ---- Theme tokens (Add Product form-এর সাথে হুবহু মিল রেখে) ----
  ...
  const updateTier = (id: string, patch: Partial<EditPricingTier>) => {
    setTiers((prev) => prev.map((t) => (t.id === id ? { ...t, ...patch } : t)));
  };

  // ekta notun file select/drop hoyar sathe sathe Cloudinary te upload shuru
  const handleNewFiles = (newFiles: File[]) => {
    const entries: EditImage[] = newFiles.map((file) => ({
      id: crypto.randomUUID(),
      file,
    ...
    entries.forEach((entry) => {
      uploadImageToCloudinary(entry.file as File)
        .then((result) => {
          setImages((prev) =>
            prev.map((img) =>
              img.id === entry.id
            ...
            {({ close }) => (
              <>
                ...
                <Modal.Header>
                  ...
                  >
                    Update the listing details for{" "}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: textMuted }}
                      >
                        Support for high-resolution PNG, JPG, or PDF
                        specification sheets. Max size 20MB per file.
                      </p>
                    </div>
                ...
                <Modal.Footer>
                  ...
                  <Button
                    ...
                    isDisabled={isAnyImageUploading || isSaving}
